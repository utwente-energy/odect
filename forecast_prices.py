...
startTime = int(start.timestamp()) - 7*24*3600
endTime = int(start.timestamp())

avg_err = 0
last_err = 0


## Code to correct for recent deviations, but this is still broken
# try:
	# # actual data
...
# Load scalars
scaler_in = joblib.load("training/scalar_in_prices_forecast") 
scaler_out = joblib.load("training/scalar_out_prices_forecast") 
...
while start <= end - datetime.timedelta(days = 1):
	...
	r_fc = influx_read(url, dbname, startTime-(24*3600), endTime-(24*3600), measurement, field, condition)
...
# perform the actual prediction
	for i in range(0, 10):
		model = keras.models.load_model('training/ml_prices_forecast_'+str(i)+'.keras')
		...
		for val in result:
			...
			tags = {'country': 'NL', 'type': 'electricity', 'model': str(i)}
			values = {'euro': value, 'euro_zonneplan': price_zp}
